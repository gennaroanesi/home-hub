...
- **Household inventory** — *phase 1 built:* modular `homeInventoryItem` (category, owner via `ownerPersonId` / `pregnancyId` for the baby on the way, `WISHLIST | OWNED | SOLD | GIVEN_AWAY`, estimated vs paid price) plus per-category detail models (`homeInventoryClothing`: size/color/type/season; `homeInventoryConsumable`: unit, low-stock threshold, restock list, expiry). `/inventory` page, Janet tools (`list_inventory`, `manage_inventory_item`, `adjust_inventory_quantity` — auto-restocks low consumables onto the shopping list), wishlist total on the dashboard Baby card. Wishlist is private (no public registry page). *Next:* barcode `pendingScan` flow from *Inventory* below, photos, mobile screen, and moving `pregnancyId` items to the baby's `ownerPersonId` at delivery.
...
- **Who's at the door** — UniFi Protect doorbell ring (or motion-person on the front camera) → Rekognition lookup against the existing face collection → push notification with the person's name when matched, "unknown person" otherwise. Reuses the heavy machinery already in the repo: `homeHubFaceCollection`, per-`homePerson` enrollments, `faceDetector` Lambda, expo push fan-out.
  ...
  - **Latency budget:** Rekognition is 1-2 s. End-to-end from press to push is ~3 s. Plenty fast for a "heads up, it's family at the door" notification, **not** fast enough for "auto-unlock for recognized family" — that lives in the future device-control HIGH path with its own gate.
