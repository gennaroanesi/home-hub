  ...
  .schema({
    ...
    homeTask: a
      .model({
        ...
        // For one-time tasks (`recurrence == null`) this is the single
        // due moment and `isCompleted` / `completedAt` close the row.
        // For recurring tasks (`recurrence != null`) this mirrors the
        // currently-open occurrence's `scheduledFor` so legacy callers
        // (the agent's task tools, older clients) keep reading sane
        // values; the source of truth is `homeTaskOccurrence`.
        dueDate: a.datetime(),
    ...
    // ── Task occurrence (recurring tasks only) ─────────────────────────
    // One row per cycle of a recurring homeTask. Generated by the
    // taskOccurrenceAction Lambda when a previous occurrence is closed,
    // or lazily for legacy recurring tasks the first time a client
    // takes an action on them. Closed = completedAt OR skippedAt set;
    // exactly one row per series should be open at a time.
    homeTaskOccurrence: a
      .model({
        taskId: a.id().required(),
        scheduledFor: a.datetime().required(),
        completedAt: a.datetime(),
        completedByPersonId: a.id(),
        skippedAt: a.datetime(),
        skipReason: a.string(),
      })
    ...
    // ── Recurring task occurrence actions ───────────────────────────────
    // Single endpoint for the three recurring-task lifecycle actions —
    // see amplify/functions/task-occurrence-action/handler.ts for the
    // semantics. Web and mobile both call this so completing a
    // recurring task means the same thing on every surface.
    taskOccurrenceActionResponse: a.customType({
      ok: a.boolean().required(),
      occurrenceId: a.id(),
      nextOccurrenceId: a.id(),
      message: a.string(),
    }),
    taskOccurrenceAction: a
      .mutation()
      .authorization((allow) => [
        allow.group("home-users"),
        allow.authenticated("identityPool"),
      ...
      .arguments({
        action: a.string().required(), // "COMPLETE" | "SKIP" | "UNCOMPLETE"
        taskId: a.id().required(),
        byPersonId: a.id(),
        reason: a.string(),
      })
      .returns(a.ref("taskOccurrenceActionResponse"))
      .handler(a.handler.function(taskOccurrenceAction)),
  })
  .authorization((allow) => [
